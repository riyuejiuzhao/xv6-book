...
`mknod`在文件系统中创建了一个文件，但是这个文件是空的。同时，这个文件的元数据把文件标记为设备文件，并且记录了主设备号和副设备号（就是`mknod`中的两个参数）。主设备号和副设备号唯一标识出一个内核记录中的设备。当一个进程打开这个文件的时候，内核将`read`、`write`这两个系统调用转交给内核中的设备管理而不是文件系统。
